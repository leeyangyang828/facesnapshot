...
class DragDropView: UIView,Droppable{
...
//        debugPrint("View：dropOverInfoInRect：\(rect.origin.x),\(rect.origin.y)_|\(rect.width),\(rect.height)")
    ...
    func dropComplete(_ dragInfo : AnyObject,dropInfo:AnyObject?, atRect rect: CGRect) -> Void{
        ...
        if (model?.imageName != nil){
            appDelegate.imageProcessController?.addImgToPan(model: model!, rect: rect)
        }
        model = nil
        
//        let imgProcessViewController = ImageProcessViewController(nibName: "ImageProcessViewController", bundle: nil)
//        imgProcessViewController.addImgToPan(model: model!, rect: rect)
